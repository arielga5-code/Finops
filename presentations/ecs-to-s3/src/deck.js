...
slides.push({ ops: S(o => {
  ...
  const risks = [
    { a: C.red,    t: 'משפטי · קבילות ואישור', r: 'שם הקובץ ותאריך היצירה משתנים בהעתקה. בית משפט עשוי לדרוש אסמכתא שהתוכן לא שונה.', m: 'manifest חתום SHA-256 לכל אובייקט — שם מקורי, שם יעד ותאריך מקור — יחד עם S3 Object Lock. הפתרון מחייב אישור מפורש של הלשכה המשפטית לפני ההעתקה הראשונה.', blocking: true },
    ...
    { a: C.orange, t: 'אפליקציה · שינוי מצביעים', r: 'האפליקציות מצביעות היום לנתיב ב-ECS. מעבר ל-S3 מחייב פיתוח בצד היישום, לא רק העברת דאטה.', m: 'אפיון מול צוותי הדימות ו-Verint, הערכת מאמץ פיתוח, ותקופת ביניים שבה שתי הכתובות פעילות במקביל.' },
    { a: C.orange, t: 'מיליוני קבצים קטנים',   r: 'הקורפוס הוא מיליוני קבצים קטנים; שמות ומבנה התיקיות ב-S3 שונים, ואין מיפוי אחד-לאחד.', m: 'מיפוי שמות ומבנה מוסכם מראש, ואימות של צוותי האפליקציה שכל מסמך נגיש בשמו החדש. נדרש זמן צוות ייעודי.' },
    { a: C.orange, t: 'ביצועים · זמני אחזור',  r: 'אחזור מהענן איטי מאחזור מ-ECS מקומי. לטנציה בשליפה עלולה לפגוע בשירות למשתמש הקצה.', m: 'מדידת זמני אחזור מול ה-SLA הקיים לפני התחייבות, חיבור Direct Connect ולא אינטרנט, וקאשינג מקומי לפריטים חמים.' },
    { a: C.orange, t: 'כלי ההעברה · POC',      r: 'אין כלי העתקה מוכח בסדר גודל כזה; קבצים עלולים להיכשל בהעברה בלי שהדבר יתגלה.', m: 'POC קצר על 1–2 TB עם AWS DataSync: ולידציה מובנית, דוח חריגים לכל ריצה, ומחיקה מ-ECS רק לאחר אימות מלא.' },
  ];
  const w = 3.55, gap = 0.44, h = 1.94;
  risks.forEach((k, i) => {
    const col = i % 3, row = Math.floor(i / 3);
    const x = G.W - G.M - w - col * (w + gap);
    const y = 2.2 + row * (h + 0.26);
    rect(o, x, y, w, h, { fill: C.card, line: k.blocking ? '5A2A38' : C.border, r: 0.05 });
    rect(o, x + w - 0.46, y + 0.24, 0.13, 0.13, { fill: k.a });
    txt(o, he(k.t), { x: x + 0.28, y: y + 0.16, w: w - 0.84, h: 0.28, size: 12.5, bold: true, color: k.a, align: 'right', rtl: true, valign: 'middle' });
    txt(o, he(k.r), { x: x + 0.28, y: y + 0.48, w: w - 0.56, h: 0.56, size: 11, color: C.text, align: 'right', rtl: true, valign: 'top', lh: 1.16 });
    rect(o, x + 0.28, y + 1.08, w - 0.56, 0.01, { fill: '2C3A54' });
    txt(o, he('מיטיגציה: ' + k.m), { x: x + 0.28, y: y + 1.15, w: w - 0.56, h: 0.72, size: 10.5, color: C.muted, align: 'right', rtl: true, valign: 'top', lh: 1.16 });
  });

  footnote(o, 'שני הסיכונים באדום חוסמים התחלה: בלי אישור הלשכה המשפטית ובלי אישור DPO ורגולטור אין העתקה ראשונה. ארבעת הכתומים נסגרים ב-POC ובאפיון מול צוותי האפליקציה.', C.red);
})});
