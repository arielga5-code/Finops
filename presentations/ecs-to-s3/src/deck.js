...
slides.push({ bg: C.bgDeep, ops: S(o => {
  ...
  txt(o, he('העתקת מסמכי דימות ושיחות לענן'), {
...
slides.push({ ops: S(o => {
  ...
  txt(o, he('העתקת מסמכי Commit ל-AWS S3'), { x: xR + 0.34, y: y + 0.28, w: w - 1.44, h: 0.42, size: 16.5, bold: true, color: C.text, align: 'right', rtl: true, valign: 'middle' });
  ...
  footnote(o, 'אין צורך באישור תקציב סופי לרכש בישיבה זו. הנתון החסר להחלטה מלאה הוא גודל קורפוס ה-Commit — כמה TB באמת יפונו.', C.cyan);
})});
...
// -------------------------------------------------- 6 · Tier correction
slides.push({ ops: S(o => {
  header(o, 'מה שההשוואה מפספסת', 'השוואנו מול S3 Standard — אבל דימות ושיחות הם דאטה ארכיוני', C.purple, 25);
  ...
  card(o, G.M, 2.5, 5.7, 1.32, C.green, 'מה שמוזיל את הענן',
    'מסמכי Commit נכתבים פעם אחת ונקראים לעיתים נדירות. ב-Glacier Instant Retrieval, בזמן אחזור של מילישניות, העלות יורדת לכ-$25K — פי 4.6 זול מ-On-Prem, ולא יקר ממנו ב-30%.', { size: 12 });

  card(o, G.M, 4.06, 5.7, 1.6, C.red, 'מה שמייקר אותו',
  ...
  footnote(o, 'הנתון שחסר להכרעה: פילוח גודל-אובייקט ותדירות אחזור של קורפוס ה-Commit. הוא שקובע איזו שכבה נכונה, והוא אינו קיים היום. המחירים הם מחירון רשימה, ללא הנחות נפח או התחייבות.', C.purple);
})});
...
slides.push({ ops: S(o => {
  ...
  const bens = [
    { a: C.green,  t: 'פינוי שטח מיידי',   b: 'העתקת קורפוס ה-Commit מפנה שטח ב-ECS תוך שבועות. זו האפשרות היחידה שמשפיעה על הניצולת לפני דצמבר 2026.' },
...
slides.push({ ops: S(o => {
  ...
  const recs = [
    { n: '01', a: C.cyan,   t: 'אישור עקרוני להעתקת מסמכי Commit ל-AWS S3',
