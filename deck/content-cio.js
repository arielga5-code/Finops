...
const core = [
...

  {
    ...
    chart: {
      type: "line",
      legend: true,
      colors: [COLORS.text, COLORS.warn, COLORS.danger],
      inline: {
        cats: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
        series: [
          { name: "Actual", vals: [234115, 218324, 261455, 234947, 236775, 256207, 283554, null, null, null, null, null] },
    ...
    speakerNotes: [
      "THE UPPER BOUND. Same start, same method, faster rate.",
      "",
      "Rates from the May-July window, as published in the July briefing:",
      "  Azure  +13.3% per month",
      "  AWS     +5.8% per month",
      "  GCP     +$1.1K per month",
      "",
      "Carried from July's actual 283,554:",
      "  Aug 314,904   Sep 349,873   Oct 388,919   Nov 432,559   Dec 481,378",
      "  481,378 x 12 = $5.78M",
      "",
      "WHAT IT COSTS IF IT HAPPENS",
      "  December run-rate   5,776,533  vs  4,011,077   = +1,765,456 a year",
      ...
      "",
      "Note the Azure rate here is v33's 13.3%, measured off the invoiced May",
      "figure of 145,254. On the consumption basis this deck uses, May was 152,263",
      "and the same window gives 10.8% a month. The 13.3% is deliberately the",
      "harsher of the two, which is what an upper bound should be.",
      "",
      "The trigger to watch: if August lands above roughly 300K, we are on the red",
...

  {
    ...
    eyebrow: "Tagging",
    accent: COLORS.cyan,
    title: "Tag the request, not just the resource",
    note: "Resource tags answer which subscription. Only per-request metadata answers which use case.",
    left: {
      heading: "Foundation tags",
      items: ["owner", "application", "environment", "cost_center", "business_unit"],
    },
    centre: {
      title: "The gateway injects them",
      sub: "Every AI request carries its own metadata",
      chips: ["use_case", "agent_id", "workflow_id", "ai_model", "ai_provider", "mcp_tool", "session_id", "request_id"],
      foot: "The gateway is the only place these can be made mandatory.",
    },
    right: {
    ...
    speakerNotes: [
      ...
      "on a request. One Bedrock key served $25,288 of Claude in July across an",
      "unknown number of use cases. No tag on a resource can split that.",
      "",
      "WHAT CHANGES",
      "Every AI call goes through the gateway anyway, for budgets and quotas. The",
      ...
      "gateway configuration, done once, enforced in CI/CD.",
    ].join("\n"),
  },

  {
    ...
    eyebrow: "Enforcement",
    accent: COLORS.danger,
    title: "Azure Budget will not stop the spend",
    note: "A budget reports. Only the gateway can refuse a request.",
    tables: [
      {
        title: "The control model",
        sub: "Every layer enforced in API Management, per subscription key",
        cols: [
          { label: "Control layer", w: 2.7 },
          { label: "Enforced at", w: 2.3 },
          { label: "What it prevents", w: 3.6 },
        ],
        rows: [
          ["TPM / RPM", "User and application", "Minute-scale bursts"],
          ["Daily token quota", "Application", "A broken job running for hours"],
          ["Monthly token quota", "Application / cost centre", "Overrun of the approved budget"],
          ["Model allowlist", "Application", "Unplanned jump to an expensive model"],
          ["Max output tokens", "Every request", "Runaway answers and agent loops"],
          ["Global deployment ceiling", "Model / backend", "Organisation-wide blast radius"],
        ],
      },
    ],
    stats: [
    ...
    speakerNotes: [
      ...
      "roughly 8-24 hours late, the budget is evaluated about once a day, and",
      "breaching it does not stop consumption. By the time it fires, the money is",
      "spent. APIM token quota is the enforcement point; the budget is the late",
      "financial control.",
      "",
      ...
      "  counter-key on context.Subscription.Id, estimate-prompt-tokens=\"true\"",
      "  so an oversized prompt is blocked before it reaches the model.",
      "  TPM breach returns 429; quota breach returns 403.",
      "Add rate-limit-by-key, cap max_output_tokens, disable streaming and tool",
      "calling where not needed, and cap concurrency for agents.",
      "",
      "HIERARCHY: low TPM per user; TPM plus quotas per application; cumulative",
      "quota per cost centre; a ceiling on each expensive model; sandbox well",
      ...
      "cost centre, model, environment and owner as dimensions. Alert on an",
      "abnormal token rate over 5 and 15 minutes; 70% of quota to the owner; 85%",
      "to FinOps with a rate reduction; 100% automatic block; a jump in the",
      "output-to-input ratio; and repeated 429s, 403s or agent retries.",
    ].join("\n"),
  },
